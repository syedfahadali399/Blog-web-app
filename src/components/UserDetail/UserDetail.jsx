...
function UserDetail() {
  ...
  return (
    <>
       {loading? <Loader/>:
           <div className="animate-in fade-in zoom-in-95 duration-500 space-y-12 max-w-7xl mx-auto w-full">
            <div className="bg-white border border-slate-200 rounded-b-[3rem] p-6 sm:p-8 md:p-12 relative overflow-hidden">
                  ...
                  <div className="flex-1 text-center md:text-left space-y-6">
                    ...
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 pt-2 text-sm font-bold text-slate-400">
                      ...
                      <div className="flex items-center gap-2 bg-slate-100 px-3 py-1 rounded-full text-slate-600">
                        <Clock size={16} /> Joined At  {userData.joined_at}
                      </div>
                    </div>
                  </div>
               </div>
            ...
            <div className="space-y-8 p-4 sm:p-8">
              <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-8 text-center md:text-left">Published Works</h2>                
                ...
                {userAllDetail.filter(p => p.id).map(blog => (
                  <article key={blog.id} className="bg-white border border-slate-200 rounded-3xl overflow-hidden hover:shadow-xl hover:shadow-indigo-500/5 transition-all flex flex-col group">
                    <div className="relative overflow-hidden aspect-video w-full">
                      <img src={blog.social_image} className="w-full h-full object-cover hover:scale-105 transition-transform duration-1000" />
                    </div>
                    <div className="p-6 flex flex-col flex-1 space-y-4 justify-between">
                      <div className="space-y-3">
                        <div className="flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-widest text-indigo-500">
                          {blog.tag_list && blog.tag_list.length > 0 ? (
                            blog.tag_list.slice(0, 2).map((tag, idx) => (
                              <span key={idx} className="bg-indigo-50 px-2 py-0.5 rounded text-[10px] tracking-normal font-bold">#{tag}</span>
                            ))
                          ) : (
                            <span>Work</span>
                          )}
                      ...
                      <div className="pt-4 mt-auto flex items-center justify-between border-t border-slate-50">
                        ...
                        <Link to={`/blog/${blog.id}`}
                          className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors flex items-center gap-2 font-bold text-sm cursor-pointer"
                        >
